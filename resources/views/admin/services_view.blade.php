...
<body>
    ...
    <p>Schedule: {{ $data->schedule->date }}</p>
    @foreach ($data->schedule->timeslot as $dates)
        <p>Start Time: {{ $dates->start_time }}</p>
        <p>End Time: {{ $dates->end_time }}</p>
        <p>Status: {{ $dates->status }}</p>
    @endforeach
</body>
